...
get_tasks() {
    ...
    mapfile -s 3 -t lines <<< "$(task minimal)"
    for num in ${!lines[@]}; 
    do 
        #Skip footer
        (( $num == ${#lines[@]}-1 )) && break
        local columns=(${lines[num]})
        local id=${columns[0]}
        #Skip empty lines :S and current active task
        [[ -z $id || $current_task_id == $id ]] && continue
...
add_new_task(){
    ...
    local description=
    local ret=
    for project in $(task _projects); do projects+="$project#"; done
    projects="${projects%#}"
